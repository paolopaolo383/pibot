...
class ledder(commands.Cog, name="ledder"):
    ...
    async def on_message(self, message):
        ...
        def check(msg):
            ...
            if message.author.id == msg.author.id and int(msg.content)>0 and int(msg.content)<count+1:
                return True
            else:
                return False
        ...
        try:
            re = await self.client.wait_for(event='message', timeout=60.0, check=check)
        except asyncio.TimeoutError:
            await message.channel.send("시간이 지났습니다", reference=message)
            return

        ledderarray = numpy.array([line, line, line, line, line])
                ...
                if ledderarray[i, j] > 0:
                    ...
                    if ledderarray[i, j] == 1:  # 안지나간 부분
                        if i == 0 or i == le-1:
                            ...
                            elif ledderarray[i, j + 1] == 1 and not ledderarray[i, j - 1] == 1:  # 오른쪽이랑 아래
                                content[i, j] = fragments[2]
                            elif not ledderarray[i, j + 1] == 1 and ledderarray[i, j - 1] == 1:  # 왼쪽이랑 아래
                        ...
                        if i == 0 or i == le-1:
                            ...
                            elif ledderarray[i, j + 1] > 0 and not ledderarray[i, j - 1] :  #위 있는 오른쪽이랑 아래
                                if ledderarray[i - 1, j] == 2:
                                    content[i, j] = fragments[8]
                                else:
                                    content[i, j] = fragments[14]

                            elif not ledderarray[i, j + 1] > 0 and ledderarray[i, j - 1] > 0 and ledderarray[i-1,j-1]==2:  #왼쪽위 왼쪽이랑 아래
                                if ledderarray[i + 1, j] == 2:
                                    content[i, j] = fragments[15]
                                else:
                                    content[i, j] = fragments[9]
                            elif not ledderarray[i, j + 1] > 0 and ledderarray[i, j - 1] >0 and not ledderarray[i-1,j-1]==2:  #왼쪽위 왼쪽이랑 아래
                                if ledderarray[i - 1, j] == 2:
                                    content[i, j] = fragments[9]
                                else:
                                    content[i, j] = fragments[15]
